...
keys = [
    ...
    Key([mod], "x", lazy.spawn("wlogout -n -m 0"), desc="Exit Qtile menu"),
...
def text_cut(text):
    text = re.sub(".*Brave", "Brave", text)
    text = re.sub(".*Teams", "Teams", text)
    text = re.sub(".*Slack", "Slack", text)
    for string in [" - Firefox", " - VSCodium", " - client"]: 
